class CardTypesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @card_type.save
        format.html { redirect_to card_type_url(@card_type), notice: "Card type was successfully created." }
  ...
  def update
    respond_to do |format|
      if @card_type.update(card_type_params)
        format.html { redirect_to card_type_url(@card_type), notice: "Card type was successfully updated." }
  ...
  private
    ...
    def card_type_params
      params.require(:card_type).permit(:card_type_name)
    end
end
